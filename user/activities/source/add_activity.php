...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($document_name == '' || $category_name == '' || $sub_category_name == '' || !isset($_FILES['file']) || $_FILES['file']['error'] !== UPLOAD_ERR_OK) {
        ...
        if (!in_array($imageFileType, ["jpg", "png", "jpeg", "gif", "pdf"])) {
        ...
        } elseif ($_FILES["file"]["size"] > 5000000000) {
            ...
            if ($insert_result) {
                // Get the last inserted document ID to use for calendar
                $document_id = mysqli_insert_id($connection);

                // Insert into calendar table
                $calendar_insert_query = "INSERT INTO calendar (document_id, date_time_reminder) 
                                           VALUES ('$document_id', '$date_time_reminder')";
                $calendar_insert_result = mysqli_query($connection, $calendar_insert_query);

                if (!$calendar_insert_result) {
                    echo "Error inserting date_time_reminder into calendar: " . mysqli_error($connection);
                } else {
